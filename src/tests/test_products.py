...
class TestCatalogView(TestCase):
    ...
    def test_catalog_displays_products_in_categories(self):
        ...
        product_names = [link.get_text().strip() for link in product_links if 'View All' not in link.get_text()]
    ...
    def test_catalog_product_links_are_correct(self):
        ...
        product_links = soup.find_all('a', class_='product-link')
        product_detail_links = [link for link in product_links if 'View All' not in link.get_text()]
        
        # Check that links contain product detail URLs
        for link in product_detail_links:
            href = link.get('href')
            assert href and ('/catalog/' in href or 'product_detail' in href)  # Adjust based on your URL pattern
